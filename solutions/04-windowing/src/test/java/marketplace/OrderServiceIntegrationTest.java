...
class OrderServiceIntegrationTest extends FlinkIntegrationTest {
    ...
    @Test
    @Timeout(60)
    public void createOrdersForPeriodTable_shouldCreateTheTable() throws Exception {
        TableResult result = testKit.registerTemporaryTable(customerOrdersForPeriodTableName, orderService::createOrdersForPeriodTable);

        String status = result.collect().next().getFieldAs(0);
        assertEquals("Command completed successfully.", status);

        assertTrue(
            testKit.tableExists(customerOrdersForPeriodTableName),
            "Could not find the table: "+customerOrdersForPeriodTableName
        );
    ...
    public void streamCustomerPurchasesDuringPeriod_shouldStreamRecordsToTheTable() throws Exception {
        // Create a temporary orders table.
        testKit.tableEnvironment.useCatalog("flink-table-api-java");
        testKit.tableEnvironment.useDatabase("marketplace");
        testKit.createTemporaryTable(ordersTableName, orderTableDescriptor);

        // Create the output table.
        testKit.registerTemporaryTable(customerOrdersForPeriodTableName, orderService::createOrdersForPeriodTable);
        ...
        // will do the trick.
        testKit.insertInto(ordersTableName, Collections.singletonList(new OrderBuilder().build()));

        // Execute the method being tested.
        testKit.registerTemporaryStatement(orderService.streamOrdersForPeriod(windowSize));

        // Fetch the results.
        TableResult results = testKit.tableEnvironment.from(customerOrdersForPeriodTableName)
            .select($("*"))
            .execute();
